...
struct FanBladeMeasure {
    ...
    inline void operator()(const T x[X_N], T z[RZ_N]) const {
        ...
        z[idx::CENTER_X] = img_pts_jet[RuneKeyPointsIndex::CENTER].x();
        z[idx::CENTER_Y] = img_pts_jet[RuneKeyPointsIndex::CENTER].y();
    }
    template<typename T>
    inline Eigen::Transform<T, 3, Eigen::Isometry> fan_pose(const T x[X_N]) const {
        ...
        Eigen::Transform<T, 3, Eigen::Isometry> pose_in_rune;
        pose_in_rune.translation() << 0, 0, 0;

        Eigen::Quaternion<T> q_yaw_in_rune(
            Eigen::AngleAxis<T>(x[idx::YAW], Eigen::Vector3<T>::UnitZ())
        );
        Eigen::Quaternion<T> q_pitch_in_rune(Eigen::AngleAxis<T>(0, Eigen::Vector3<T>::UnitY()));
        Eigen::Quaternion<T> q_roll_in_rune(Eigen::AngleAxis<T>(roll, Eigen::Vector3<T>::UnitX()));
        pose_in_rune.linear() =
            (q_yaw_in_rune * q_pitch_in_rune * q_roll_in_rune).toRotationMatrix();
        Eigen::Transform<T, 3, Eigen::Isometry> rune_in_odom =
            Eigen::Transform<T, 3, Eigen::Isometry>::Identity();
        rune_in_odom.translation() << x[idx::CX], x[idx::CY], x[idx::CZ];
        Eigen::Quaternion<T> q_yaw_rune_in_odom(
        ...
        Eigen::Transform<T, 3, Eigen::Isometry> pose_in_odom = rune_in_odom * pose_in_rune;
        return pose_in_odom;
    }
