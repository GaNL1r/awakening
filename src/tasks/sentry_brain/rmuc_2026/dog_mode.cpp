...
DogMode::DogMode(rcl::RclcppNode& rcl_node, rcl::TF& rcl_tf, const YAML::Node& config) {
    _impl = std::make_unique<Impl>(rcl_node, rcl_tf, config);
}
DogMode::~DogMode() noexcept {
    _impl.reset();
}
void DogMode::update_armor_target(const auto_aim::ArmorTarget& target_in_big_yaw) {
    _impl->update_armor_target(target_in_big_yaw);
}
void DogMode::update_gobal_state(const SentryRefereeReceive& packet) {
